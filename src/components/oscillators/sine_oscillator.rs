//! Port of `synth.components.oscillators.SineOscillator`.
...
/// A sine-wave oscillator backed by the pre-computed sine lookup table.
pub struct SineOscillator {
    core: OscillatorCore,
    /// Cached `&'static` reference to the sine table (resolved once at construction).
    table: &'static [f64],
}

impl SineOscillator {
    /// Constructs a SineOscillator with a given sample rate.
    ///
    /// # Panics
    ...
    pub fn new(sample_rate: f64) -> Self {
        SineOscillator {
            ...
            table: &lookup_tables::tables().sine,
        }
    }
}
...
impl AudioComponent for SineOscillator {
